defmodule Day12P1 do
  ...
  def solve(filename) do
    ...
    traverse(grid, start_position, end_position, %{
      start_position => %{d: 0, parent: nil}
    })
  end
  ...
  defp traverse(grid, {nx, ny} = position, {ex, ey} = end_pos, costs, visited \\ MapSet.new()) do
    if position == end_pos do
      ...
      {min_pos, _} = for {key, %{d: d}} <- Map.to_list(costs), reduce: {{0, 0}, 10000000000000} do
        {pos, distance} ->
